...
export default function HomePage() {
  ...
  useEffect(() => {
    if (selectedVideo && currentTime > 0) {
      localStorage.setItem(`sonora_playback_${selectedVideo.id}`, currentTime.toString());
    }
  ...
  const handleVideoSelect = useCallback((video) => {
    ...
    if (isLive) {
      ...
      const savedData = localStorage.getItem(`sonora_playback_${video.id}`);
      if (savedData) {
    ...
    <ContextMenu key={result.id}>
      <ContextMenuTrigger>
        <motion.div
          ...
          className="bg-gradient-card glass-dark rounded-2xl overflow-hidden hover-lift cursor-pointer group relative w-full h-[300px] md:h-[300px] flex flex-col hover:glow-purple animate-float"
        ...
        >
          <div onClick={(e) => {
          ...
          }} className="relative aspect-video h-[180px] rounded-t-2xl overflow-hidden">
            <Image 
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent group-hover:from-black/80 transition-all duration-300 flex items-center justify-center">
              ...
              >
                <div className="bg-gradient-primary p-3 rounded-full glow-blue">
            ...
            {result.isLive && (
              <motion.span 
                ...
                className="absolute top-3 left-3 bg-gradient-secondary text-white text-xs px-3 py-1.5 rounded-full font-semibold shadow-lg glow-pink"
              >
                ● LIVE
              </motion.span>
            )}
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="absolute top-3 right-3 z-10 glass p-2 rounded-full backdrop-blur-md hover:bg-white/20 transition-all duration-200"
            >
              <Heart
                fill={isVideoLiked(result.id) ? "#f093fb" : "none"}
                stroke={isVideoLiked(result.id) ? "#f093fb" : "white"}
                size={18}
                className={isVideoLiked(result.id) ? "animate-pulse-glow" : ""}
              />
            </motion.button>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <motion.button
                  ...
                  className="absolute top-2 left-2 z-10 glass p-1.5 rounded-full md:hidden backdrop-blur-md hover:bg-white/20 transition-all duration-200"
          ...
          <div className="p-5 flex-grow flex flex-col justify-between bg-gradient-to-b from-transparent to-black/20">
            <div>
              <h3 className="font-semibold text-sm mb-3 text-white line-clamp-2 h-10 group-hover:text-transparent group-hover:bg-gradient-primary group-hover:bg-clip-text transition-all duration-300">{result.title}</h3>
              <p className="text-xs text-gray-300 mb-2 font-medium">{result.channelName}</p>
            </div>
            <div className="flex justify-between items-center text-xs">
              <motion.span 
                className="text-gray-400 bg-white/10 px-2 py-1 rounded-full transition-all duration-200 hover:bg-gradient-primary hover:text-white cursor-default"
                whileHover={{ scale: 1.05, y: -2 }}
                transition={{ type: "spring", stiffness: 400, damping: 17 }}
              >
                {result.views}
              </motion.span>
              {result.uploadedAt !== 'N/A' && 
                <motion.span 
                  className="text-gray-400 bg-white/10 px-2 py-1 rounded-full transition-all duration-200 hover:bg-gradient-secondary hover:text-white cursor-default"
                  whileHover={{ scale: 1.05, y: -2 }}
                  transition={{ type: "spring", stiffness: 400, damping: 17 }}
                >
                  {result.uploadedAt}
            ...
            <div className="flex items-center space-x-2 sm:space-x-4 flex-shrink-0">
              {user ? (
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <motion.button 
                      className="text-white font-semibold hover:text-gray-300 transition-colors duration-200 flex items-center cursor-pointer text-xs sm:text-sm glass px-3 py-2 rounded-full backdrop-blur-md hover:bg-white/10"
                      whileHover={{ scale: 1.05, y: -2 }}
                      whileTap={{ scale: 0.95 }}
                      transition={{ type: "spring", stiffness: 400, damping: 17 }}
                    >
                ...
                <>
                  <motion.div
                    ...
                    transition={{ type: "spring", stiffness: 400, damping: 17 }}
                  >
                    <Link href="/login" className="text-xs sm:text-sm text-white hover:text-gray-300 transition-colors duration-200 glass px-3 py-2 rounded-full backdrop-blur-md hover:bg-gradient-primary hover:text-white" target="_blank" rel="noopener noreferrer">
                      Sign In
                    </Link>
                  </motion.div>
                  ...
                  <motion.div
                    ...
                    transition={{ type: "spring", stiffness: 400, damping: 17 }}
                  >
                    <Link href="/signup" className="text-xs sm:text-sm text-white hover:text-gray-300 transition-colors duration-200 glass px-3 py-2 rounded-full backdrop-blur-md hover:bg-gradient-secondary hover:text-white" target="_blank" rel="noopener noreferrer">
                      Create Account
                    </Link>
                  </motion.div>
                </>
  ...
  const renderNavItem = (label, icon, navKey) => {
    ...
    return (
      <motion.button
        className={`
          flex items-center w-full py-3 px-4 rounded-xl glass transition-all duration-300 cursor-pointer group relative overflow-hidden
          ${isSelected && !isDisabled 
            ? 'bg-gradient-primary text-white glow-blue' 
            : 'text-gray-300 hover:bg-white/10 hover:text-white hover:glow-purple'
          }
      ...
      >
        ...
        <div className={`mr-3 group-hover:animate-pulse ${isSelected ? 'animate-pulse-glow' : ''}`}>
          {icon}
        </div>
        ...
        {/* Hover indicator */}
        <motion.div
          className="absolute right-2 w-1 h-6 bg-gradient-primary rounded-full opacity-0 group-hover:opacity-100"
          initial={{ scale: 0 }}
          whileHover={{ scale: 1 }}
        ...
        {isLoading ? (
          ...
          >
            ...
            <motion.aside
              ...
              className="w-72 glass-dark backdrop-blur-xl border-r border-white/10 p-6 hidden md:flex flex-col h-full relative overflow-hidden"
            >
              ...
              <div className="relative z-10 flex flex-col h-full">
                <motion.div
                  ...
                  className="mb-8"
                >
                  <h1 className="text-4xl font-bold text-white mb-2">Sonora</h1>
                  <div className="h-1 w-16 bg-gradient-primary rounded-full glow-blue" />
                </motion.div>
                ...
                >
                  ...
                  <motion.button
                    className={`flex items-center w-full py-3 px-4 rounded-xl glass transition-all duration-300 cursor-pointer group ${
                      selectedNavItem.startsWith('playlist-') 
                        ? 'bg-gradient-primary text-white glow-blue' 
                        : 'text-gray-300 hover:bg-white/10 hover:text-white hover:glow-purple'
                    } ${!user ? 'opacity-50' : ''}`}
                    ...
                    whileTap={{ scale: 0.98 }}
                  >
                    <Library className="mr-3 group-hover:animate-pulse" size={20} />
                    <span className="font-semibold flex-1">Your Library</span>
                    <motion.div
                      animate={{ rotate: isLibraryOpen ? 180 : 0 }}
                      transition={{ duration: 0.3 }}
                    >
                  ...
                  <AnimatePresence>
                    {isLibraryOpen && user && (
                      ...
                      >
                        <motion.button
                          className="flex items-center w-full py-2 px-3 rounded-lg glass text-gray-300 hover:bg-gradient-accent hover:text-white transition-all duration-300 cursor-pointer group"
                          onClick={() => handleNavClick('Create Playlist')}
                          whileHover={{ scale: 1.02, x: 2 }}
                          whileTap={{ scale: 0.98 }}
                        >
                          <PlusCircle className="mr-2 group-hover:rotate-90 transition-transform duration-300" size={16} />
                            ...
                            <ContextMenu key={playlist.id}>
                              <ContextMenuTrigger>
                                <motion.button
                                  ...
                                  className={`flex items-center w-full py-2 px-3 rounded-lg transition-all duration-300 cursor-pointer group ${
                                    selectedPlaylist && selectedPlaylist.id === playlist.id
                                      ? 'bg-gradient-secondary text-white glow-pink'
                                      : 'text-gray-400 hover:bg-white/10 hover:text-white hover:glow-purple'
                                  }`}
                ...
                {/* Bottom section with user info or login prompt */}
                <motion.div
                  initial={{ y: 20, opacity: 0 }}
                  animate={{ y: 0, opacity: 1 }}
                ...
                >
                  {user ? (
                    ...
                    >
                      <p className="text-sm text-gray-300 mb-2">Sign in to access your library</p>
                      <motion.div
                      ...
                      >
                        <Button 
                          className="w-full bg-gradient-primary hover:bg-gradient-secondary text-white text-sm transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/30"
            ...
            <AnimatePresence>
              {selectedVideo && (
                ...
                >
                    ...
                    <div className="flex items-center justify-between w-full sm:w-auto sm:space-x-8">
                      <div className="flex items-center space-x-4 sm:space-x-8">
                        ...
                        >
                          <SkipBack 
                            size={20} 
                            className={`cursor-pointer ${isLiveVideo ? 'text-gray-600' : 'text-gray-400 hover:text-white hover:drop-shadow-glow'} transition-all duration-300 sm:w-6 sm:h-6`} 
                        ...
                        {isLiveVideo ? (
                          ...
                          >
                            {isPlaying ? (
                              <Pause size={28} className="cursor-pointer text-white hover:text-gray-300 transition-all duration-300 hover:drop-shadow-glow sm:w-8 sm:h-8" onClick={togglePlayPause} />
                            ) : (
                              <Play size={28} className="cursor-pointer text-white hover:text-gray-300 transition-all duration-300 hover:drop-shadow-glow sm:w-8 sm:h-8" onClick={togglePlayPause} />
                            )}
                            <motion.div
                      ...
                      <div className="flex items-center space-x-4 sm:space-x-6">
                        ...
                        >
                          <Heart
                            ...
                            stroke="white"
                            size={20}
                            className="transition-all duration-300 sm:w-6 sm:h-6 hover:drop-shadow-glow"
                          />
                        ...
                        <div className="hidden sm:flex items-center space-x-6">
                          ...
                          >
                            ...
                            <motion.input
                              ...
                              className="w-20 accent-white transition-all duration-300 hover:accent-blue-400"
                              whileHover={{ scale: 1.05 }}
                          ...
                          >
                            <List 
                              size={20} 
                              className="cursor-pointer text-white hover:text-gray-300 transition-all duration-300 hover:drop-shadow-glow" 
                  ...
                  >
                    {isLiveVideo ? (
                      <div className="flex justify-between items-center text-xs text-gray-400 mt-1 glass rounded-lg p-3">
                        <motion.span 
                        ...
                        <span className="text-purple-300">Listening for: {formatLiveTime(liveListeningTime)}</span>
                      </div>
                    ) : (
                      <>
                        ...
                        <div className="flex justify-between text-xs text-gray-400 mt-2 px-2">
                          ...
                          <span className="text-purple-300 font-medium">{duration ? formatTime(duration) : '--:--'}</span>
                        </div>
                      </>
                    )}
                  </motion.div>
                  <div className="h-0 overflow-hidden">
                    <YouTubePlayer
                      ...
                      onReady={(event) => {
                        ...
                        if (!isLiveVideo) {
                          const savedTime = localStorage.getItem(`sonora_playback_${selectedVideo.id}`);
                          if (savedTime) {
                  ...
                  >
                    ...
                    <div className="flex justify-end space-x-2">
                      <motion.div
                        ...
                        transition={{ type: "spring", stiffness: 400, damping: 17 }}
                      >
                        <Link href="/login" className="px-3 py-1 bg-white text-black rounded text-sm hover:bg-gray-300 transition-all duration-300 hover:shadow-lg" target="_blank" rel="noopener noreferrer">
                          Sign In
                        </Link>
                      </motion.div>
                      <motion.div
                        ...
                        transition={{ type: "spring", stiffness: 400, damping: 17 }}
                      >
                        <Link href="/signup" className="px-3 py-1 bg-gray-600 text-white rounded text-sm hover:bg-gray-700 transition-all duration-300 hover:shadow-lg hover:shadow-gray-500/30" target="_blank" rel="noopener noreferrer">
                          Create an Account
                        </Link>
                      </motion.div>
                    </div>
